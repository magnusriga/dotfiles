return {
  -- Setup `nvim-lspconfig`.
  {
    ...
    opts = {
      ...
      servers = {
        vtsls = {
          ...
          keys = {
            ...
            -- Server vs. Client.
            -- =================================================================
            -- - `lua_ls` and `vstls` are SERVERS, not clients.
            -- - `vstls` server started with `vtsls --stdio`, following which client can
            --   communicate with `vstls` server via standard input|output.
      ...
      -- `opts.setup`.
      -- =======================================
      -- - `opts.setup[server](server_opts)` is called right before loading language server,
      --   i.e. before calling `require('nvim-lspconfig').server.setup(server_opts)`,
      --   both from `plugins/lsp/init.lua` > `nvim-lspconfig` spec's `config`-function.
      -- - `server_opts` passed into `setup.server(..)` is same as
      --   `server_opts` passed into `require('nvim-lspconfig').setup(server_opts)`.
      -- - Thus `server_opts` contains entries for every server defined
      --   accross all `nvim-lspconfig` specs' `opts.servers`.
      --
      ...
      setup = {
        ...
        -- `client.commands`.
        -- =======================================
        -- - See note above, including on Code Actions example
        -- - `client.commands` are manually populated, client-side functions,
        --   executed when language server sends command to client with name
        --   matching key in `client.commands`, e.g. in code action, code lenses, etc.

        -- =======================================
        -- `_typescript.moveToFileRefactoring`.
        -- =======================================
        -- - Custom client-side command, allowing code sections to be moved to given file.
        -- - Interactive interface, where user specifies target file.
        -- - Activated with code action.
