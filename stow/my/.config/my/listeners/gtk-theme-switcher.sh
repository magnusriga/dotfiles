...
apply_theme() {
  ...
  if [ "$THEME_PREF" -eq 1 ]; then
    ...
    "$HOME/.local/bin/matugen" image "$(cat ~/.cache/my/hyprland-dotfiles/current_wallpaper)"
    ...
    "$HOME/.local/bin/matugen" image "$(cat ~/.cache/my/hyprland-dotfiles/current_wallpaper)" -m "light"
