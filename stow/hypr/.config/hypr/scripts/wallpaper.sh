...
# -----------------------------------------------------
# Sync the generated PNG into SDDM theme Backgrounds dirs so the login
# screen shows the same wallpaper as hyprlock + the desktop. Each SDDM
# theme the repo installs has a `Backgrounds/current_wallpaper.png` that
# its config references (see dotfiles/usr/share/sddm/themes/...).
# -----------------------------------------------------
for sddm_theme_bg in \
  /usr/share/sddm/themes/hyprlock/Backgrounds/current_wallpaper.png \
  /usr/share/sddm/themes/sddm-astronaut-theme/Backgrounds/current_wallpaper.png; do
  if [ -f "$currentwallpaper" ] && [ -d "$(dirname "$sddm_theme_bg")" ]; then
    sudo install -m644 "$currentwallpaper" "$sddm_theme_bg" 2>/dev/null \
      ...
      || _writeLog "Could not sync login wallpaper to $sddm_theme_bg"
  fi
done
unset sddm_theme_bg
